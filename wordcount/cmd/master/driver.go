...
// The actual implementation is in wordcount/master/run.go
	...
	if err := master.CreateRunDirectories(runPath); err != nil {
	...
	if err := master.ShardInput(config.InputPath, runPath, config.M); err != nil {
...
func runBaseline(config RunConfig, runPath string, summary *RunSummary) {
	...
	if err := master.Shuffle(config.M, config.R, runPath); err != nil {
...
func runMappersSample(config RunConfig, runPath string) error {
	...
	for i := 0; i < config.M; i++ {
		...
		go func(mapID int) {
			...
			cmd := master.BuildMapperCommand("sample", mapID, shardPath, outDir, config.SampleRate, 0, "", config.Seed, "wordcount")
			if err := master.RunCommand(cmd, filepath.Join(runPath, "logs", fmt.Sprintf("mapper_%d_sample.log", mapID))); err != nil {
				mu.Lock()
...
func runMappers(config RunConfig, runPath string, planPath string) error {
	...
	for i := 0; i < config.M; i++ {
		...
		go func(mapID int) {
			...
			cmd := master.BuildMapperCommand("execute", mapID, shardPath, outDir, 0, config.R, planPath, config.Seed, "wordcount")
			if err := master.RunCommand(cmd, filepath.Join(runPath, "logs", fmt.Sprintf("mapper_%d_execute.log", mapID))); err != nil {
				mu.Lock()
				errors = append(errors, fmt.Errorf("mapper %d failed: %w", mapID, err))
				mu.Unlock()
			}
...
func runMergeUnsalt(config RunConfig, runPath string) error {
	...
	return master.RunCommand(cmd, filepath.Join(runPath, "logs", "merge_unsalt.log"))
}
...
func buildMergeUnsaltCommand(inputsGlob, outPath string) *exec.Cmd {
	...
	return exec.Command("go", "run", "./wordcount/cmd/merge_unsalt",
		"--inputs-glob", inputsGlob,
		"--out", outPath,
	)
}
...
// runReducers launches all reducers in parallel
func runReducers(config RunConfig, runPath string) error {
	...
	for r := 0; r < config.R; r++ {
		...
		go func(reduceID int) {
			...
			cmd := master.BuildReducerCommand(reduceID, inputsPath, outPath, "wordcount", 0.85, 0)
			if err := master.RunCommand(cmd, filepath.Join(runPath, "logs", fmt.Sprintf("reducer_%d.log", reduceID))); err != nil {
				mu.Lock()
...
// writeWordCountMetrics writes WordCount run summary metrics to a JSON file
func writeWordCountMetrics(metricsPath string, summary RunSummary) error {
