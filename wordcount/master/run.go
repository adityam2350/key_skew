...
func runMappersSample(config RunConfig, runPath string) error {
	...
	for i := 0; i < config.M; i++ {
		...
		go func(mapID int) {
			...
			cmd := commonmaster.BuildMapperCommand("sample", mapID, shardPath, outDir, config.SampleRate, 0, "", config.Seed, "wordcount")
...
func runMergeUnsalt(config RunConfig, runPath string) error {
	...
	return commonmaster.RunCommand(cmd, filepath.Join(runPath, "logs", "merge_unsalt.log"))
}
...
func runReducers(config RunConfig, runPath string) error {
	...
	for r := 0; r < config.R; r++ {
		...
		go func(reduceID int) {
			...
			cmd := commonmaster.BuildReducerCommand(reduceID, inputsPath, outPath, "wordcount", 0.85, 0)
			if err := commonmaster.RunCommand(cmd, filepath.Join(runPath, "logs", fmt.Sprintf("reducer_%d.log", reduceID))); err != nil {
				mu.Lock()
