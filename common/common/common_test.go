...
// ============================================================
// hashing.go: CanonicalKey, ExtractBaseKey, CreateSaltedKey, PartitionKey
// ============================================================
...
func TestCanonicalKey_MalformedSaltedKey(t *testing.T) {
	// A 3-element array is an invalid salted key format
	_, err := CanonicalKey([]interface{}{"a", float64(1), "extra"})
	if err == nil {
		t.Error("expected error for malformed salted key, got nil")
	}
}
...
func TestPartitionKey_SaltedKeyInRange(t *testing.T) {
	R := 8
	// Use float64 salt to match JSON-unmarshaled format expected by CanonicalKey
	key := []interface{}{"the", float64(2)}
	p, err := PartitionKey(key, R)
	if err != nil {
...
func TestPartitionKey_UnsaltedVsSaltedDiffer(t *testing.T) {
	...
	salted, err := PartitionKey([]interface{}{"the", float64(0)}, R)
	if err != nil {
		t.Fatal(err)
	}
