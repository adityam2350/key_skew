...
func RunPageRank() {
	...
	if originalGraphPath != "" {
		...
		if err := master.CopyFile(originalGraphPath, runGraphPath); err != nil {
		...
		if _, err := os.Stat(metaPath); err == nil {
			...
			if err := master.CopyFile(metaPath, runMetaPath); err == nil {
	...
	if err := master.CopyFile(config.InputPath, iter0Path); err != nil {
	...
	for iter := 1; iter <= config.Iterations; iter++ {
		...
		if err := master.Shuffle(config.M, config.R, iterDir); err != nil {
	...
	finalRanksPath := filepath.Join(finalDir, "ranks.jsonl")
		if err := master.CopyFile(currentInput, finalRanksPath); err != nil {
...
func runPageRankMappers(config PageRankConfig, iterDir string) error {
	...
	for i := 0; i < config.M; i++ {
		...
		go func(mapID int) {
			...
			cmd := master.BuildMapperCommand("execute", mapID, shardPath, outDir, 0, config.R, "", 0, "pagerank")
			if err := master.RunCommand(cmd, filepath.Join(iterDir, "..", "..", "logs", fmt.Sprintf("mapper_%d_execute.log", mapID))); err != nil {
				mu.Lock()
...
func runPageRankReducers(config PageRankConfig, iterDir string) error {
	...
	for r := 0; r < config.R; r++ {
		...
		go func(reduceID int) {
			...
			cmd := master.BuildReducerCommand(reduceID, inputsPath, outPath, "pagerank", config.Damping, config.NumNodes)
			if err := master.RunCommand(cmd, filepath.Join(iterDir, "..", "..", "logs", fmt.Sprintf("reducer_%d.log", reduceID))); err != nil {
				mu.Lock()
